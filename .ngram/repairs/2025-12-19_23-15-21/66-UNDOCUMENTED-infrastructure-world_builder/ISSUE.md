...
**Target:** `tests/infrastructure/world_builder`
**Problem:** No documentation mapping (2 files)

## CRITICAL: Check for existing docs first

Before creating anything, search for existing documentation:
- `grep -r "tests/infrastructure/world_builder" docs/` - check if this path is mentioned in existing docs
...
4. If not found:
   ...
   b. Read the code in `tests/infrastructure/world_builder` to understand what it does
