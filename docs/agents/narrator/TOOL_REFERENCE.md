...
Definitive output schema for narrator responses. For full type definitions, see `docs/schema/SCHEMA.md` and `engine/models/`.
...
Mutation payloads must validate against Pydantic models in `engine/models/`.
...
4. Mutations validate against `engine/models/`.
