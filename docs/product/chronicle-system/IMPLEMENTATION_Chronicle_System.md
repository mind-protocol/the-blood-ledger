...
This doc is the canonical Chronicle implementation reference. The former
technical pipeline doc is now a pointer here to avoid duplication.
...
```
docs/product/chronicle-system/
  (no runtime implementation yet - design only)
```
...
|------|---------|----------------------|-------|--------|
| data/Distributed-Content-Generation-Network/Blood Chronicle System.md | Source spec for this module | N/A | N/A | DRAFT |
...
|---------|------------|---------|
| N/A | N/A | No runtime implementation yet |
...
|----------|--------|---------|-----------|
| Chronicle System | Design docs | Runtime code | N/A |
...
The Chronicle system relies on a multi-stage technical pipeline to generate,
compose, and render video content from player gameplay data.
...
The core of narrative generation is driven by a structured LLM prompt,
ensuring cinematic and character-authentic output.
...
Voice generation is handled by a Text-to-Speech (TTS) system, with specific
voices mapped to different characters and the narrator.
...
FFmpeg is used to compose the video from images, audio tracks, and effects,
following a defined timeline.
